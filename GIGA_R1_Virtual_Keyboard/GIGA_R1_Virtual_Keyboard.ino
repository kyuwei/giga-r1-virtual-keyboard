...
//  La Radxa configuree en FR interprete correctement chaque position.
// ================================================================
struct KeyMapping {
  char    c;
  uint8_t keycode;
...
void drawScreen(String ip) {
  ...
  Display.textSize(2);
  Display.text("USB-C  ->  port USB Radxa Q6A", 55, 318);
  ...
  while (client.connected()) {
    ...
    if (c == '\n') {
      ...
      if (currentLine.length() == 0) {
        ...
        client.print("<title>Clavier Radxa</title>");
        client.print("<style>");
        ...
        client.print("#vkb{margin:8px 0;}");
        client.print(".kb-row{display:flex;justify-content:center;gap:3px;margin:3px 0;}");
        client.print(".kb-key{min-width:34px;height:40px;padding:2px;font-size:15px;background:#16213e;color:#fff;border:1px solid #2a3a60;border-radius:6px;cursor:pointer;}");
        client.print(".kb-key:hover,.kb-key:active{background:#4fc3f7;color:#000;}");
        client.print(".kb-sp{min-width:120px;font-size:12px;}");
        client.print("</style></head><body>");
        client.print("<h2>&#9000; Clavier Virtuel &rarr; Radxa Q6A</h2>");
        ...
        client.print("var mode='"); client.print(azertyMode ? "azerty" : "qwerty"); client.print("';");
        client.print("var APO=String.fromCharCode(39),DQ=String.fromCharCode(34),SP=' ';");
        client.print("var LAYOUTS={");
        client.print("azerty:[['&','\\u00E9',DQ,APO,'(','-','\\u00E8','_','\\u00E7','\\u00E0'],");
        client.print("['a','z','e','r','t','y','u','i','o','p'],");
        client.print("['q','s','d','f','g','h','j','k','l','m'],");
        client.print("['w','x','c','v','b','n',',',SP]],");
        client.print("qwerty:[['1','2','3','4','5','6','7','8','9','0'],");
        client.print("['q','w','e','r','t','y','u','i','o','p'],");
        client.print("['a','s','d','f','g','h','j','k','l'],");
        client.print("['z','x','c','v','b','n','m',SP]]};");
        client.print("function buildKbd(){");
        client.print("var vkb=document.getElementById('vkb');vkb.innerHTML='';");
        client.print("LAYOUTS[mode].forEach(function(row){");
        client.print("var r=document.createElement('div');r.className='kb-row';");
        client.print("row.forEach(function(k){");
        client.print("var b=document.createElement('button');");
        client.print("b.textContent=(k===SP?'Espace':k);");
        client.print("b.className='kb-key'+(k===SP?' kb-sp':'');");
        client.print("b.onclick=function(){fetch('/text?val='+encodeURIComponent(k)).then(function(){s.textContent='Touche: '+(k===SP?'Espace':k);});};");
        client.print("r.appendChild(b);});vkb.appendChild(r);});}");
        client.print("buildKbd();");
